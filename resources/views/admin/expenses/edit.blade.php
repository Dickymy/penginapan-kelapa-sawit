...
<div class="">
    ...
    <form method="POST" action="{{ route('admin.expenses.update', $expense) }}" class="bg-white rounded-lg shadow-sm border border-gray-100 p-6 space-y-4">
        ...
        <div>
            ...
            <input type="date" name="expense_date" id="expense_date" value="{{ old('expense_date', $expense->expense_date->toDateString()) }}"
        ...
        <div>
            ...
            <input type="number" name="amount" id="amount" value="{{ old('amount', $expense->amount) }}" min="1"
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3"
                      class="w-full border-gray-300 rounded-md text-sm @error('description') border-red-500 @enderror">{{ old('description', $expense->description) }}</textarea>
            @error('description') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
        </div>

        <div class="flex items-center gap-3 pt-2">
            <button type="submit" class="px-4 py-2 bg-primary-600 text-white rounded-md text-sm hover:bg-primary-700">Perbarui</button>
