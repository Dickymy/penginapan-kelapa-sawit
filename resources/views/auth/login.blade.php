...
<div class="max-w-md mx-auto px-4 py-12">
    ...
    <div class="bg-white border border-gray-100 rounded-2xl shadow-[0_20px_50px_rgba(8,_112,_184,_0.07)] p-6 md:p-8">
        ...
        @if ($errors->any())
            <x-alert type="error" message="Email atau kata sandi yang Anda masukkan tidak sesuai." />
        @endif
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-4" x-data="{ submitting: false }" @submit="submitting = true">
            ...
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('password.request') }}" class="text-sm text-primary-600 hover:text-primary-800">Lupa kata sandi?</a>
            </div>
        ...
        {{-- Google Login --}}
        <a href="{{ route('auth.google') }}" class="w-full inline-flex items-center justify-center px-4 py-3.5 border border-gray-200 rounded-xl text-sm font-bold text-gray-700 bg-white hover:bg-gray-50 hover:-translate-y-1 hover:shadow-md transition-all duration-300">
    ...
    <p class="mt-6 text-center text-sm text-gray-600">
        Belum punya akun? <a href="{{ route('register') }}" class="text-primary-600 hover:text-primary-800 font-medium">Daftar sekarang</a>
    </p>
</div>
@endsection
