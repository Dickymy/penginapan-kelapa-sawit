...
<div class="max-w-2xl mx-auto px-4 py-8" x-data="paymentPage()">
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-6 mb-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            ...
            <div>
                ...
                <p class="font-bold text-primary-600 text-xl">Rp{{ number_format($payment->gross_amount, 0, ',', '.') }}</p>
            </div>
            <div>
    ...
    <div class="text-center space-y-4">
        ...
        @if(!config('midtrans.is_production'))
        <div class="pt-4 border-t border-gray-100 max-w-md mx-auto">
            <form action="{{ route('booking.simulate-payment', $booking->booking_code) }}" method="POST">
                @csrf
                <button type="submit" class="inline-flex items-center text-orange-600 bg-orange-50 hover:bg-orange-100 px-6 py-2.5 rounded-lg font-medium transition text-sm">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
                    </svg>
                    Bypass Pembayaran (Uji Coba Cepat)
                </button>
            </form>
            <p class="mt-2 text-xs text-orange-700 opacity-80">Tombol ini hanya muncul di mode Sandbox/Local untuk mempercepat pengujian Anda.</p>
        </div>
        @endif
    </div>
</div>
